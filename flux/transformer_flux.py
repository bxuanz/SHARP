...
def compute_rs_kappa_rds(current_timestep: float, alpha: float = 4.0) -> float:
    """
    Rational Decay Scheduler (RDS) - 有理衰减调度器
    专为遥感极高频细节设计的平滑快速衰减函数
    
    Args:
        current_timestep: 当前归一化时间步 [0, 1]
        alpha: 衰减控制因子 (alpha >= 1)，建议设置为 4.0
    """
    alpha = max(1.0, alpha)
    return current_timestep / (alpha - (alpha - 1.0) * current_timestep)
...
def get_1d_rotary_pos_embed(
    ...
    Args:
        ...
        dype (`bool`, *optional*, defaults to False):
            If True, enable SHARP timestep-aware positional scaling.
        current_timestep (`float`, *optional*, defaults to 1.0):
            Current timestep for SHARP positional scaling, normalized to [0, 1] where 1 is pure noise.
...
class FluxPosEmbed(nn.Module):
    ...
    def set_timestep(self, timestep: float):
        """Set the current SHARP timestep, normalized to [0, 1] where 1 is pure noise."""
...
class FluxTransformer2DModel(
    ...
    """
    The Transformer model introduced in Flux with SHARP positional scaling support.
    ...
    Args:
        ...
        dype (`bool`, defaults to `True`):
            Whether to enable SHARP timestep-aware positional scaling.
    """
    ...
    @classmethod
    def from_sharp_checkpoint(cls, pretrained_model_name_or_path: str, **kwargs):
    ...
    def forward(
        ...
        self.pos_embed.set_timestep(timestep.item())
